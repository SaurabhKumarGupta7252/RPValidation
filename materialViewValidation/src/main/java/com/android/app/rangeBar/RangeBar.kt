...
class RangeBar : View {
    ...
    private var mThumbCenterPointColor = DEFAULT_CONNECTING_LINE_COLOR
    private var mThumbBoundarySize = DEFAULT_CIRCLE_BOUNDARY_SIZE_DP
    private var mThumbCenterPointSize = DEFAULT_CENTER_CIRCLE_BOUNDARY_SIZE_DP
    ...
    private val mDefaultWidth =
        TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 250f, mDisplayMetrices).toInt()
    private val mDefaultHeight =
        TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 75f, mDisplayMetrices).toInt()
    ...
    private var mActiveThumbCenterPointColor = 0
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        bundle.putInt("CENTER_CIRCLE_BOUNDARY_COLOR", mThumbCenterPointColor)
        bundle.putFloat("CIRCLE_BOUNDARY_WIDTH", mThumbBoundarySize)
        bundle.putFloat("CENTER_CIRCLE_BOUNDARY_WIDTH", mThumbCenterPointSize)
    ...
    public override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            mThumbCenterPointColor = bundle.getInt("CENTER_CIRCLE_BOUNDARY_COLOR")
            mThumbBoundarySize = bundle.getFloat("CIRCLE_BOUNDARY_WIDTH")
            mThumbCenterPointSize = bundle.getFloat("CENTER_CIRCLE_BOUNDARY_WIDTH")
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        if (isRangeBar) {
            ...
            mLeftThumb!!.init(
                ...
                mThumbColorLeft,
                mThumbBoundaryColor,
                mThumbCenterPointColor,
                mThumbBoundarySize,
                mThumbCenterPointSize,
                mMinPinFont,
                mMaxPinFont,
                mArePinsTemporary
            )
    ...
    /**
     * Set the color of the Thumb Center Point.
     *
     * @param thumbCenterPointColor Integer specifying the boundary color of the ticks.
     */
    fun setThumbCenterPointColor(thumbCenterPointColor: Int) {
        mThumbCenterPointColor = thumbCenterPointColor
        createPins()
    }
    ...
    /**
     * Set the size of the Thumb Center Point.
     *
     * @param thumbCenterPointSize Integer specifying the boundary size of ticks.
     * Value should be in DP
     */
    fun setThumbCenterPointSize(thumbCenterPointSize: Int) {
        mThumbCenterPointSize = thumbCenterPointSize.toFloat()
        createPins()
    }
    ...
    override fun setEnabled(enabled: Boolean) {
        if (!enabled) {
            ...
            mThumbCenterPointColor = DEFAULT_BAR_COLOR
            ...
            mThumbCenterPointColor = mActiveThumbCenterPointColor
    ...
    private fun rangeBarInit(context: Context, attrs: AttributeSet?) {
        ...
        try {
            ...
            mTickHeight = ta.getDimension(
                R.styleable.RangeBar_mrb_tickHeight,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_TICK_HEIGHT_DP,
                    mDisplayMetrices
                )
            )
            mBarWeight = ta.getDimension(
                R.styleable.RangeBar_mrb_barWeight,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_BAR_WEIGHT_DP,
                    mDisplayMetrices
                )
            )
            mThumbSize = ta.getDimension(
                R.styleable.RangeBar_mrb_thumbSize,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CIRCLE_SIZE_DP,
                    mDisplayMetrices
                )
            )
            mThumbBoundarySize = ta.getDimension(
                R.styleable.RangeBar_mrb_thumbBoundarySize,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CIRCLE_BOUNDARY_SIZE_DP,
                    mDisplayMetrices
                )
            )
            mThumbCenterPointSize = ta.getDimension(
                R.styleable.RangeBar_mrb_thumbCenterPointSize,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CENTER_CIRCLE_BOUNDARY_SIZE_DP,
                    mDisplayMetrices
                )
            )
            mConnectingLineWeight = ta.getDimension(
                R.styleable.RangeBar_mrb_connectingLineWeight,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CONNECTING_LINE_WEIGHT_DP,
                    mDisplayMetrices
                )
            )
            mExpandedPinRadius = ta.getDimension(
                R.styleable.RangeBar_mrb_pinRadius,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_EXPANDED_PIN_RADIUS_DP,
                    mDisplayMetrices
                )
            )
            mPinPadding = ta.getDimension(
                R.styleable.RangeBar_mrb_pinPadding,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_PIN_PADDING_DP,
                    mDisplayMetrices
                )
            )
            mBarPaddingBottom = ta.getDimension(
                R.styleable.RangeBar_mrb_rangeBarPaddingBottom,
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, DEFAULT_BAR_PADDING_BOTTOM_DP,
                    mDisplayMetrices
                )
            )
            ...
            mThumbColor = ta.getColor(
                R.styleable.RangeBar_mrb_thumbColor,
                DEFAULT_CONNECTING_LINE_COLOR
            )
            mThumbColorLeft = ta.getColor(
                R.styleable.RangeBar_mrb_leftThumbColor,
                mThumbColor
            )
            mThumbColorRight = ta.getColor(
                R.styleable.RangeBar_mrb_rightThumbColor,
                mThumbColor
            )
            mThumbBoundaryColor = ta.getColor(
                R.styleable.RangeBar_mrb_thumbBoundaryColor,
                DEFAULT_CONNECTING_LINE_COLOR
            )
            mThumbCenterPointColor = ta.getColor(
                R.styleable.RangeBar_mrb_thumbCenterPointColor,
                DEFAULT_CONNECTING_LINE_COLOR
            )
            ...
            mActiveThumbCenterPointColor = mThumbCenterPointColor
            mTickDefaultColor =
                ta.getColor(R.styleable.RangeBar_mrb_tickDefaultColor, DEFAULT_TICK_COLOR)
            mActiveTickDefaultColor = mTickDefaultColor
            mTickColors =
                getColors(ta.getTextArray(R.styleable.RangeBar_mrb_tickColors), mTickDefaultColor)
            mActiveTickColors = ArrayList(mTickColors)
            mTickLabelColor =
                ta.getColor(R.styleable.RangeBar_mrb_tickLabelColor, DEFAULT_TICK_LABEL_COLOR)
            mActiveTickLabelColor = mTickLabelColor
            mTickLabelSelectedColor = ta.getColor(
                R.styleable.RangeBar_mrb_tickLabelSelectedColor,
            ...
            mTickDefaultLabel =
                if (mTickDefaultLabel != null) mTickDefaultLabel else DEFAULT_TICK_LABEL
            val mConnectingLineColor = ta.getColor(
                R.styleable.RangeBar_mrb_connectingLineColor,
                DEFAULT_CONNECTING_LINE_COLOR
            )
            ...
            mMinPinFont = ta.getDimension(
                R.styleable.RangeBar_mrb_pinMinFont,
                DEFAULT_MIN_PIN_FONT_SP * density
            )
            mMaxPinFont = ta.getDimension(
                R.styleable.RangeBar_mrb_pinMaxFont,
                DEFAULT_MAX_PIN_FONT_SP * density
            )
            mTickLabelSize = ta.getDimension(
                R.styleable.RangeBar_mrb_tickLabelSize,
    ...
    private fun getPinValue(tickIndex: Int): String {
        ...
        val tickValue =
            if (tickIndex == tickCount - 1) mTickEnd else tickIndex * mTickInterval + mTickStart
    ...
    companion object {
        ...
        private const val DEFAULT_CENTER_CIRCLE_BOUNDARY_SIZE_DP = 0f
